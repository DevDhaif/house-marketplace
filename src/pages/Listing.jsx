...
function Listing() {
  ...
  return (
    <main className="min-h-screen p-4 ">
      <Swiper
        slidesPerView={1}
        className="outline-1 outline outline-blue-300 rounded-md shadow-md mb-4 bg-blue-50"
        navigation={true}
      ...
      {sharedLink && (
        <p className="right-0 px-2 text-white py-1 mt-2 mx-1 rounded-md bg-blue-500 absolute">
          Link Copied
        </p>
      )}
      <div className="flex relative flex-col gap-y-4">
        ...
        <div className="flex justify-start gap-x-4 ">
          <p className="text-xs bg-blue-500 text-white px-2 py-0.5  rounded-md w-16">
            {listing.type === "rent" ? t("forRent") : t("forSale")}
          </p>
          {listing.offer && (
            <p className="text-xs bg-gray-800 text-white px-2 py-0.5  rounded-md w-24">
        ...
        <button onClick={() => navigate(-1)}>
          <RiArrowLeftSLine className="fill-blue-500 w-10 h-10  bg-white rounded-full" />
        </button>
      </div>
    </main>
  );
